...
func TestLoadConfigReplacesConfigurationAndClearsThreads(t *testing.T) {
	db, cleanup := setupDB(t)
	defer cleanup()
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/configs", strings.NewReader(`{"name":"replace-base","description":"base"}`))
	req.Header.Set(echo.HeaderContentType, echo.MIMEApplicationJSON)
	rec := httptest.NewRecorder()
	e.ServeHTTP(rec, req)
	...
	_, err = db.Exec(ctx, `INSERT INTO teams (id, name) VALUES ('team-2','Team Two')`)
	require.NoError(t, err)
	_, err = db.Exec(ctx, `INSERT INTO agents (id, name, role, team_id) VALUES ('agent-2','Agent Two','worker','team-2')`)
	require.NoError(t, err)
	_, err = db.Exec(ctx, `INSERT INTO threads (id, created_by_agent, status, team_id) VALUES ('thread-1','agent-2','open','team-2')`)
	require.NoError(t, err)

	req = httptest.NewRequest(http.MethodPost, "/api/v1/configs/"+created.ID+"/load", nil)
	rec = httptest.NewRecorder()
	e.ServeHTTP(rec, req)
	...
	err = db.QueryRow(ctx, `SELECT COUNT(*) FROM agents`).Scan(&count)
	require.NoError(t, err)
	assert.Equal(t, 1, count)
